...
export type UseGesturesOptions = {
  ...
  validKeyGestures?: ValidKeyGestures;
  validRockerGestures?: ValidRockerGestures;
  validWheelGestures?: ValidWheelGestures;
  validLineGestures?: ValidLineGestures;
...
export function useGestures(options: UseGesturesOptions = {}) {
  ...
  const moveGesture = useCallback(
    (event: MouseEvent | WheelEvent, diagonal: boolean = false): void => {
      ...
      if (newDirection === lastDirection) {
        gestureRef.current.line.dirPoints[
          gestureRef.current.line.dirPoints.length - 1
        ] = next;
      } else if (
        ...
        if (gestureRef.current.line.possibleDirs) {
          gestureRef.current.line.possibleDirs =
            gestureRef.current.line.possibleDirs[newDirection];
        }

        if (gestureRef.current.line.possibleDirs || !validLineGestures) {
    ...
    [onRefreshLine, validLineGestures, endGesture],
  );
  ...
  const wheelGesture = useCallback(
    async (event: WheelEvent): Promise<void> => {
      ...
      const dir = determineWheelDirection(event.deltaY);
      if (validWheelGestures && !validWheelGestures[dir]) {
        return;
      }
    ...
    [endGesture, moveGesture, validWheelGestures, sendGesture, holdButton],
  );
    ...
    async (event: MouseEvent): Promise<void> => {
      ...
      if (gestureRef.current.rocker) {
        ...
        if (dirs) {
          ...
          if (!validRockerGestures || validRockerGestures?.[first]?.[second]) {
            if (syncButtonsRef.current?.timeout) {
              clearTimeout(syncButtonsRef.current.timeout);
            }
            syncButtonsRef.current = {
      ...
      if (
        ...
        (!validRockerGestures || validRockerGestures.M) &&
      ...
      const rockerEnabled =
        pressed.length === 1 &&
        (!validRockerGestures ||
          Boolean(validRockerGestures[(['L', 'M', 'R'] as const)[pressed[0]]]));
      const wheelEnabled =
        event.button === MouseButton.Right &&
        (!validWheelGestures || Boolean(validWheelGestures));
  ...
  const handleKeyDown = useCallback(
    async (event: KeyboardEvent): Promise<void> => {
      ...
      const seq = buildKeySequence(event);
      const keyIsInvalid =
        validKeyGestures &&
        ((mod === '0000' && isEditableElement(focusRef.current)) ||
          !validKeyGestures?.[mod].includes(seq));
      ...
      startGesture();
      await sendGesture(seq);

      event.preventDefault();
      event.stopPropagation();
    },
    [endGesture, validKeyGestures, startGesture, sendGesture],
  );
  ...
  const handleSyncButton = useCallback(
    ({ id, down }: SyncButtonMessage): void => {
