import React, { ComponentPropsWithoutRef, JSX, useEffect, useRef } from 'react';
...
function Line({
  ...
  useEffect(() => {
    ...
    context.save();
    context.translate(
      width / 2 - center.x / ratio,
      height / 2 - center.y / ratio,
    );
    context.beginPath();
    tip(gesture[0]);
    for (let index = 0; index < gesture.length - 1; index += 1) {
  ...
  return (
    <canvas ref={canvasReference} width={width} height={height} style={style} />
  );
}
...
export function Gesture({
  ...
  switch (gesture[0]) {
    case 'r': {
      c = (
        <Rocker
          gesture={gesture as RockerGesture}
          width={width}
          style={style}
        />
      );
    ...
    case 'w': {
      c = (
        <Wheel gesture={gesture as WheelGesture} width={width} style={style} />
      );
      break;
    }
    case 'k': {
